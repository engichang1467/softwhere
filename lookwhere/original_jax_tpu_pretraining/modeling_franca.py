...
class FeedForward(ViTBase, nn.Module):
    ...
    def __call__(self, x: Array) -> Array:
        x1, x2 = jnp.split(self.w12(x), 2, axis=-1)
...
class ViT(ViTBase, nn.Module):
    def setup(self):
        ...
        layer_fn = ViTLayer
        self.layer = [layer_fn(**self.kwargs) for _ in range(12)]
    ...
    def __call__(
        ...
        # Optionally initialize cls externally
        ...
        patch_token = x[:, 1:, :]  # (bs, num_patches, dim)
        ...
        if len(attn_aggregate) == 0:
            ...
            attn_maps = attn_maps[:, 1:]  # (bs, num_patches)
...
class Zoomer(ViTBase, nn.Module):
    def setup(self):
        ...
        layer_fn = ViTLayer
        self.layer = [layer_fn(**self.kwargs) for _ in range(3)]
        ...
        self.target_grid_size = 37  # hardcode Franca's 518x518 resolution
